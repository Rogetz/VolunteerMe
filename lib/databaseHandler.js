...
userToHandle.prototype.signUp = function(fn){
    ...
    var insertCommand = `INSERT INTO user(Email,UserName,Password,Phone,FullName) VALUES(?,?,?)`
    let userDataArray = [user.email,user.userName,user.password,user.phone,user.fullName]
...
userToHandle.updatePassword = function(userName,newPassword,fn){
    //find a way of including the newPassword inside the query
    let newHashed = md5(newPassword)
    let updateQuery = `UPDATE user SET Password = ? WHERE UserName = ?`
    db.run(updateQuery,newHashed,userName,function(err){
        if(err){
...
userToHandle.prototype.logIn = function(fn){
    ...
    user.password = hashedPassword;
    console.log(user.password)
    var searchquery = 'SELECT * FROM user WHERE UserName = ?'
    console.log(user.userName)
    ...
    db.get(searchquery,user.userName,function(err,row){
        if(err){
            console.log(`error retrieving the email data:\n ${err}`)
            fn("error retrieving the email data",null)
        }
        else{
